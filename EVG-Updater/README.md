...
```bash
EVG_Updater.exe firmware.bin [options]
```
...
EVG_Updater.exe firmware.bin

# Update device at address 5, RGB mode
EVG_Updater.exe firmware.bin --addr 5 --mode 4

# Use a different gateway
EVG_Updater.exe firmware.bin --ip 10.0.0.50

# Build and flash in one command (from Firmware/ directory)
pio run && ../EVG-Updater/EVG_Updater.exe .pio/build/genericCH32V003F4P6/firmware.bin
```
